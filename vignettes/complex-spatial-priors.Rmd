...
This shapefile is built into the package, any other shapefile should be loaded using the `readOGR()` function from [rgdal](https://cran.r-project.org/web/packages/rgdal/index.html). In this prior we assume the probability of observing a source at a specific location is either constant (inside the shapefile) or zero (outside the shapefile). When we update source locations via the Metropolis-Hastings algorithm any proposed value that is outside the shapefile is immediately rejected. This differs from the conventional geographic profiling model (the DPM model in @Verity2014) that implements the shapefile post-hoc.    

What if we have a better informed prior knowledge? For example, we might suspect our criminal to favour the northernly region of our search area, rather than the south.    
...
```
```{r, echo = FALSE}
p_uniform <- rgeoprofile_file("complex_spatial_project_uniform.rds")
   p_bias <- rgeoprofile_file("complex_spatial_project_bias.rds")
...
# saveRDS(p_bias, file = "complex_spatial_project_bias.rds")
```
### References
