...
start by producing diagnostic plots of the log-likelihood. We should do this
for each value of K to check for good MCMC behaviour

```{r}
plot_loglike_dignostic(p, K = 3)
```

we can also plot 95% quantiles over this distribution. This is not so useful
now, but will become very useful if we ever move to multiple temperature rungs
as we can plot them all together

```{r}
plot_loglike(p, K = 1)
```

we should also look at the effective sample size. We should keep this number
in mind when looking at the trace plots. For example, if we have 10,000
sampling iterations but an ESS of 80 then we really only have 80 samples from
the posterior, and so we should run it out for longer
...
next we can plot posterior 95% credible intervals of each sigma. At the moment
we get one quantile for each source even under the single sigma model - I need
to fix this
...
next is the expected population size

```{r}
plot_expected_popsize(p, K = 3)
```

Note that we often get these interesting bands of colour. This is because we
are scanning through sentinel sites in rows, leading to a repeating pattern

```{r}
plot_structure(p, divide_ind_on = TRUE)
plot_structure(p, K = 3, divide_ind_on = FALSE)
```

we can plot the true group allocation directly from the simulated data for
comparison. Note that the actual group colours are arbitrary and so might not
match up between the true matrix and the posterior matrix
```{r}
plot(sim1$record$true_qmatrix)
```

Report hitscore and use plot lorenz for the gini co-efficent.

Finished gini() function, which takes hitscores as input the same as plot_lorenz(). For example:
```{r}
