...
# PAsim <- function(n_true = 100, sigma_true = 1, long_minMax = c(0, 0.5), lat_minMax = c(0, 0.5), n_traps = 100, trap_const = 0.3, K_true = 3, single_count = F, plotting = F, plotRail = 0.05, trap_spacing = "random", trap_clusters = NULL)
#   {
#   C <- 1
#   while(C < 2)
#   {
...
#   {
#
#     trap_lat <- seq(lat_minMax[1], lat_minMax[2], (lat_minMax[2] - lat_minMax[1])/sqrt(n_traps))
...
# 	}
# 	return(list(trap_data = trap_data, source_loc = source_loc, crime_loc = crime_loc, n_obs = n_obs, n_true = n_true, sigma_true = sigma_true, tau_true = tau_true, hit_data = hit_data, miss_data = miss_data, perSource = perSource, alpha_true = alpha_true, long_minMax = long_minMax, lat_minMax= lat_minMax,
#               n_traps = n_traps, trap_rad_true = trap_rad_true, K_true = K_true, plotRail = plotRail, nega_source1 = nega_source1, nega_source2 = nega_source2, nega_source3 = nega_source3))
#   C <- C + 1
# }   else{}
#
#   }
...
# # #----------------------------------------------------------------
# # MCMC
# PAmcmc <- function(modelSigma, sentinel_data, burnin = 100, iterations = 25e2, Clusters = 1, prop_const = 5, trap_rad = 1, s_prior_shape = 1, s_prior_rate = 1)
#   {
#   start <- Sys.time()
#
...
#   }
#
#   old_posterior <- post(theta_x = old_theta[,1], theta_y = old_theta[,2], sigma = modelSigma, data = sentinel_data$trap_data, trap_rad = trap_rad, exp_pop = trap_density)
#
...
#       # new_theta[k,3] <- abs(new_theta[k,3]) for variable sigma
#
#       # calculate new joint probability
#       new_posterior <- post(theta_x = new_theta[,1], theta_y = new_theta[,2], sigma = modelSigma, data = sentinel_data$trap_data, trap_rad = trap_rad, exp_pop = new_pop_den)
#
...
#       {
#         if(a_r_rate[i] == 1)
#         {propSD <- propSD + (1 - 0.23)/sqrt(iteration_vec[i])} # Robbins Monro step
...
#     new_z <- post(theta_x = old_theta[,1], theta_y = old_theta[,2], sigma = modelSigma, data = sentinel_data$trap_data, trap_rad = trap_rad, exp_pop = new_pop_den)$z
...
#   print(time_elapsed)
#
#   return(list(rawSurface = rawSurface, joint_long = joint_long, joint_lat = joint_lat, pop_densities = pop_densities, propSD_vals = propSD_vals, long_seq  = long_seq, lat_seq = lat_seq, modelSigma = modelSigma,
#               trap_rad = trap_rad))
#             }
#
# --------------------------------------------------------------------------------------------------------------------------------
# Examples set up and running of the PA MCMC
...
# K_true.sim <- 3     # randomly draw a number of sources
# traps.sim <- 36
...
#
# trap_rad_constant <- 0.3     # Set the constant, multiplied with sigma to obtain trap radius
#
# # sim data given these values
# sim <- PAsim(n_true = true_pop.sim, trap_const = trap_rad_constant, plotting = T, sigma_true = sigma_true.sim, plotRail = 0.05, K_true = K_true.sim, lat_minMax = lat_minMax.sim, long_minMax = long_minMax.sim,
#             n_traps = traps.sim, single_count = T, trap_spacing = "uniform") # , trap_clusters = 4)
#
#
# ### Run the presence absence with the fitted sigma and number of clusters from the DPM
# pa <- PAmcmc(modelSigma = sigma_true.sim, sim, burnin = 5000, iterations = 1e4, Clusters = K_true.sim, prop_const = 10, trap_rad = trap_rad_constant*sigma_true.sim)
